...
/*
1. Gsearch seems to be the biggest driver of our businesss. Could you pull monthly
...
and brand compaigns separately. I am wondering if brand is picking up at at all. If so, this is a good story to tell.alter
*/
...
I wanto to flex our analytical muscles a little and show the board we really know our traffic sources.
*/
...
Can you pull monthly trends for Gsearch, alongside monthly trends for each of our oteher channels?
*/
...
/*
5. I'd like to tell the storu of our website performance improvements over the course of the first 8 months.
Could you pull session to order conversation rates, by month?
*/
...
/*
6. For the gsearch lander test, please estimate the revenue that test earned us, and use 
...
-- NEXT, WE'LL bring in the landing page to each session, like the last time, but restricting to home or lander-1 this time.
...
-- 0.0087 ADDITIONAL ORDER PER SESSIONS

-- FINDING THE MOST RECENT PAGEVEWS FOR GSEARCH NONBRAND WHERE TRAFFIC WAS SENT TO /HOME
...
-- 22,972 website sessions sinche the test

-- X 0.0087 incremental conversion - 202 incremental orders since 7/29
		-- roughly 4 months, so roughly 50 extra ordersr per month.


/*
7. For the landing page test you analyzed previously, it would be great to show a full conversion funnel
from each of the two pages to orders. You can use the same time period you analyzed las time (Jun 19 - Jul 28).alter
*/
...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN card_made_it = 1 THEN website_session_id ELSE NULL END) AS to_card,
...
-- Then this as a final output part 2 - click rates
...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN card_made_it = 1 THEN website_session_id END) / COUNT(DISTINCT CASE WHEN mrfuzzy_made_it = 1 THEN website_session_id END) AS mrfuzzy_click_rt,
    COUNT(DISTINCT CASE WHEN shipping_made_it = 1 THEN website_session_id END) / COUNT(DISTINCT CASE WHEN card_made_it = 1 THEN website_session_id END) AS cart_click_rt,
...
/*
8. I'd love for you to quantify the impact of out billig test, as well. Please analyze the lift generated from the test (SEP 10 - NOV 10), in terms
of revenue per billing page session, and then pull the number of billing page sessions for the past moth to understand monthly impact.
*/
...
-- $22.82 revenue per billing page seen for the old version
